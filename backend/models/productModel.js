...
async function findProductById(id){
    try{       
        ...
        const product = await customerExecuteSQL('SELECT product_name , description, weight, dimension, brand FROM product WHERE product_id =?',[parseInt(id)]);
        return ( product);
...
async function findVariants(id){
    try{       
        ...
        const variants = await customerExecuteSQL('SELECT variant_Id , SKU , image_url ,price, offer, color,size, no_stock FROM variant WHERE product_id =?',[parseInt(id)]);
        return ( variants);
